        ...
        <div className="mt-12 space-y-6">
          {(t.experience.items as ExperienceItem[]).map((item, i) => (
            ...
            >
              <div className="flex items-start gap-4 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h3 className={isDark ? "text-white" : "text-gray-900"} style={{ fontSize: "1.1rem", fontWeight: 600, lineHeight: 1.3 }}>
                        {item.company}
                      </h3>
                  ...
                  <div className={`flex flex-wrap gap-4 mt-2 ${isDark ? "text-gray-500" : "text-gray-400"}`} style={{ fontSize: "0.8rem" }}>
              ...
              <p className={isDark ? "text-gray-400" : "text-gray-600"} style={{ fontSize: "0.88rem", lineHeight: 1.8, marginBottom: "1.25rem" }}>
                {item.description}
              </p>

              <ul className="space-y-2.5 mb-5">
                {item.tasks.map((task, j) => (
                  <li key={j} className={`flex gap-3 ${isDark ? "text-gray-400" : "text-gray-600"}`} style={{ fontSize: "0.85rem", lineHeight: 1.7 }}>
